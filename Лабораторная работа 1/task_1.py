...
class Subject:
    ...
    def __init__(self, name: str, duration_hours: int):
        ...
        if duration_hours <= 0:
            raise ValueError('Продолжительность дисциплины должна быть положительным числом')
    ...
    def learn(self, hours: int) -> None:
        ...
        if hours < 0:
            raise ValueError('Количество пройденных часов должно быть неотрицательным числом')
